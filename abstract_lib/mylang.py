...
def split_sentences(phrase : str):
    return all_split(phrase, { '\n', '.', '!', '?', ';'})

def split_lexems(phrase : str):
    return list(filter(bool, all_split(phrase, { '\n', '.', '!', '?', ';', ',' })))

def geom_average(val1 : float, val2 : float):
    return math.sqrt(val1 * val2)

def little_geom_average(val1 : float, val2 : float):
    big_pow = 4
    little_pow = 2
    return (geom_average(val1, val2) ** big_pow * min(val1, val2) ** little_pow) ** (1 / (little_pow + big_pow))

def split_if(data : str, function : Callable) -> list:
    lst_dat = list(data)
    new_lstdat = []
    for char in lst_dat:
        if function(char):
